...
def test_orchestrate_today_work_returns_empty_response_when_projection_fails():
    service = AdmissionsOpsService(session_factory=lambda: None)
    tenant_id = uuid4()
    actor_user_id = uuid4()
    ...
    db = FakeDb()
    service.work_state_projector.ensure_tenant_projection = lambda tenant_id: (_ for _ in ()).throw(RuntimeError("projection failed"))

    response = service.orchestrate_today_work(db, tenant_id, actor_user_id, limit=100)

    assert response.agentRunId.startswith("non-persisted-")
    assert response.board.total == 0
    assert response.board.groups == []
    assert response.run.status == "completed"
    assert response.run.result is not None
    assert response.run.result.artifacts["persisted"] is False
    assert db.rollback_count == 1
...
def test_orchestrate_today_work_returns_non_persisted_response_when_unexpected_persistence_fails():
    service = AdmissionsOpsService(session_factory=lambda: None)
    tenant_id = uuid4()
    actor_user_id = uuid4()
    student_id = uuid4()
